---
page_title: "bytepluscc_vmp_workspace Resource - terraform-provider-bytepluscc"
subcategory: ""
description: |-
  工作区（Workspace）是 VMP 服务中采集数据和规则的抽象整合，为用户提供物理隔离或逻辑隔离的 Prometheus 能力。在 VMP 服务中可创建不同的工作区，不同工作区中的数据彼此隔离。
---

# bytepluscc_vmp_workspace (Resource)
...
```shell
$ terraform import bytepluscc_vmp_workspace.example "workspace_id"
```