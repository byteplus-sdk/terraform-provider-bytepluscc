---
page_title: "bytepluscc_vefaas_kafka_trigger Resource - terraform-provider-bytepluscc"
subcategory: ""
description: |-
  函数服务支持对接消息队列 Kafka 版。通过创建 Kafka 触发器，函数服务将作为消费者消费 Kafka 中的消息，并将消息传递给用户函数，触发函数代码逻辑。您无需关心函数服务消费消息的细节，只需编写处理消息的函数。
---

# bytepluscc_vefaas_kafka_trigger (Resource)
...
```shell
$ terraform import bytepluscc_vefaas_kafka_trigger.example "function_id|kafka_trigger_id"
```