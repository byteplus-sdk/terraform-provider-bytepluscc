...
page_title: "bytepluscc_mongodb_instance Data Source - terraform-provider-bytepluscc"
subcategory: "MongoDB"
description: |-
  Data Source schema for Byteplus::MongoDB::Instance
---

# bytepluscc_mongodb_instance (Data Source)
...
- `config_servers` (Attributes Set) ConfigServers 的信息列表。 (see [below for nested schema](#nestedatt--config_servers))
...
- `mongos` (Attributes Set) Mongos 的信息列表。 (see [below for nested schema](#nestedatt--mongos))
...
- `node_availability_zone` (Attributes Set) 添加只读节点信息，包括只读节点所在的可用区和数量。 (see [below for nested schema](#nestedatt--node_availability_zone))
...
- `nodes` (Attributes Set) 副本集实例中各节点或分片集群 Shard 分片中各节点的信息列表。 (see [below for nested schema](#nestedatt--nodes))
...
- `shards` (Attributes Set) Shards 的信息列表。 (see [below for nested schema](#nestedatt--shards))
...
- `tags` (Attributes Set) MongoDB实例的标签信息 (see [below for nested schema](#nestedatt--tags))
...
- `nodes` (Attributes Set) Shard 分片中各节点的信息列表。 (see [below for nested schema](#nestedatt--shards--nodes))
