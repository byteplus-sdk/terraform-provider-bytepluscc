...
page_title: "bytepluscc_ecs_launch_template_version Data Source - terraform-provider-bytepluscc"
subcategory: "ECS"
description: |-
  Data Source schema for Byteplus::ECS::LaunchtemplateVersion
---

# bytepluscc_ecs_launch_template_version (Data Source)
...
- `network_interfaces` (Attributes Set) 实例挂载的网卡信息。 (see [below for nested schema](#nestedatt--network_interfaces))
...
- `tags` (Attributes Set) 实例的标签信息。 (see [below for nested schema](#nestedatt--tags))
...
- `volumes` (Attributes Set) 实例绑定的云盘信息。 (see [below for nested schema](#nestedatt--volumes))
