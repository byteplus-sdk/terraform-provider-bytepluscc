...
page_title: "bytepluscc_vke_cluster Data Source - terraform-provider-bytepluscc"
subcategory: ""
description: |-
  Data Source schema for Byteplus::VKE::Cluster
---

# bytepluscc_vke_cluster (Data Source)
...
- `tags` (Attributes Set) 集群的标签。 (see [below for nested schema](#nestedatt--tags))
- `type` (String) 集群类型。
...
- `log_setups` (Attributes Set) 集群的日志选项信息。 (see [below for nested schema](#nestedatt--logging_config--log_setups))
...
- `log_topic_id` (String) 采集目标的TLS日志主题ID。 如果为空，表示对应日志的主题未被创建。
...
- `conditions` (Attributes Set) 状态条件列表 (see [below for nested schema](#nestedatt--status--conditions))
- `phase` (String) 集群状态阶段
