...
type ResourceSchema struct {
	CloudControlSchemaPath               string `hcl:"cloudcontrol_schema_path,optional"`
	CloudControlTypeName                 string `hcl:"cloudcontrol_type_name"`
...
func run(destinationPackage, generatedCodeRootDirectoryName, resourcesFilename, singularDatasourcesFilename, pluralDatasourcesFilename, importExamplesFilename string) int {
	...
	envAK := os.Getenv("BYTEPLUS_ACCESS_KEY")
	envSK := os.Getenv("BYTEPLUS_SECRET_KEY")
	envRegion := os.Getenv("BYTEPLUS_REGION")
	envEndpoint := os.Getenv("BYTEPLUS_ENDPOINT")
	...
	if ak != nil && sk != nil && endpoint != nil && region != nil {
		config := volcengine.NewConfig().
			WithRegion(*region).
			WithCredentials(credentials.NewStaticCredentials(*ak, *sk, "")).
			WithEndpoint(*endpoint)
		session, _ := session.NewSession(config)
		client := cloudcontrol.New(session)
	...
	if err := downloader.MetaSchema(); err != nil {
		g.Errorf("error loading Cloud Control Resource Provider Definition Schema: %s", err)
		return 1
	}
	...
	if err != nil {
		g.Errorf("error processing Cloud Control Resource Provider Schemas: %s", err)
		return 1
	}
...
type Downloader struct {
	client        *cloudcontrol.CloudControl
	config        Config
	metaSchema    *ccschema.MetaJsonSchema
	tempDirectory string
	ui            cli.Ui
}

func (d *Downloader) MetaSchema() error {
	d.infof("using Cloud Control Resource Provider Definition Schema %q", d.config.MetaSchema.Path)

	metaSchema, err := ccschema.NewMetaJsonSchemaPath(d.config.MetaSchema.Path)

	if err != nil {
		return fmt.Errorf("loading Cloud Control Resource Provider Definition Schema: %w", err)
	}
...
func (d *Downloader) Schemas() ([]*ResourceData, *DataSources, error) {
	if d.metaSchema == nil {
		return nil, nil, fmt.Errorf("Cloud Control Resource Provider Definition Schema not loaded")
	}
	...
	for _, schema := range d.config.ResourceSchemas {
		cfResourceSchemaFilename, cfResourceTypeName, err := d.ResourceSchema(d.client, schema)

		if err != nil {
			d.ui.Warn(fmt.Sprintf("error loading Cloud Control Resource Provider Schema for %s: %s", schema.ResourceTypeName, err))
			continue
		}

		_, _, _, err = naming.ParseCloudControlTypeName(cfResourceTypeName)

		if err != nil {
			d.ui.Warn(fmt.Sprintf("incorrect format for Cloud Control Resource Provider Schema type name: %s", cfResourceTypeName))
			continue
		}
		...
		if schema.SuppressSingularDataSourceGeneration {
			...
			singularDataSources = append(singularDataSources, &DataSourceData{
				CloudControlTypeSchemaFile: cfResourceSchemaFilename,
		...
		if schema.SuppressPluralDataSourceGeneration {
			...
			pluralDataSources = append(pluralDataSources, &DataSourceData{
				CloudControlType:          cfResourceTypeName,
		...
		resources = append(resources, &ResourceData{
			...
			GeneratedCodePathSuffix:    fmt.Sprintf("%s/%s", org, svc), // e.g. "ve/logs"
			TerraformResourceType:      tfResourceTypeName,
		})
	}
...
func (d *Downloader) ResourceSchema(client *cloudcontrol.CloudControl, schema ResourceSchema) (string, string, error) {
	resourceSchemaFilename := schema.CloudControlSchemaPath
	if resourceSchemaFilename == "" {
		filename := fmt.Sprintf("%s.json", schema.CloudControlTypeName)
	...
	if !resourceSchemaFileExists {
		...
		d.infof("downloading Cloud Control Resource Provider Schema to %q", dst)
		...
		output, err := client.DescribeResourceTypeWithContext(context.TODO(), input, func(request *request.Request) {
			request.HTTPRequest.Header.Add("Accept-Language", "en")
		})
		...
		resourceSchema, err := ccschema.NewResourceJsonSchemaPath(dst)
		...
		d.infof("using cached Cloud Control Resource Provider Schema %q", resourceSchemaFilename)
	}

	// Read the resource type name from the schema.
	resourceSchema, err := ccschema.NewResourceJsonSchemaPath(resourceSchemaFilename)
...
type ResourceData struct {
	CloudControlTypeSchemaFile string
...
func (g *Generator) GenerateResourceImportExamples(packageName, filename string, resources []*ResourceData) error {
	...
	for _, v := range resources {
		tmplData, err := shared.GenerateTemplateData(g.UI(), v.CloudControlTypeSchemaFile, shared.ResourceType, v.TerraformResourceType, v.GeneratedCodePackageName)
		if err != nil {
			return fmt.Errorf("%s: %w", v.CloudControlTypeSchemaFile, err)
		}
