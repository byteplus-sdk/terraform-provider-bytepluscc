...
var (
	ccType           = flag.String("cctype", "", "CLoud Control resource type; required")
...
func (g *Generator) Generate(packageName, schemaFilename, acctestsFilename string) error {
	...
	org, svc, res, err := naming.ParseCloudControlTypeName(g.cfType)

	if err != nil {
		return fmt.Errorf("incorrect format for Cloud Control Resource Provider Schema type name: %s", g.cfType)
	}
