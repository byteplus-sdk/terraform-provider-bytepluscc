// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package rdspostgresql

import (
	"context"

	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/generic"
	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/registry"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

func init() {
	registry.AddDataSourceFactory("bytepluscc_rdspostgresql_db_account", dBAccountDataSource)
}

// dBAccountDataSource returns the Terraform bytepluscc_rdspostgresql_db_account data source.
// This Terraform data source corresponds to the Cloud Control Byteplus::RDSPostgreSQL::DBAccount resource.
func dBAccountDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: AccountName
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Database account name. The account name rules are as follows: Length must be 2–63 characters. Can contain letters, numbers, underscores (_), or hyphens (-). Must start with a letter and end with a letter or number. Cannot start with pg_. Reserved keywords cannot be used. For all disabled keywords, see Disabled Keywords.",
		//	  "type": "string"
		//	}
		"account_name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Database account name. The account name rules are as follows: Length must be 2–63 characters. Can contain letters, numbers, underscores (_), or hyphens (-). Must start with a letter and end with a letter or number. Cannot start with pg_. Reserved keywords cannot be used. For all disabled keywords, see Disabled Keywords.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: AccountPassword
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Database account password. The password rules are as follows: Length must be 8–32 characters. Must contain any three of the following: uppercase letters, lowercase letters, numbers, special characters. Supported special characters: !@#$%^*()\u0026_+-=.",
		//	  "type": "string"
		//	}
		"account_password": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Database account password. The password rules are as follows: Length must be 8–32 characters. Must contain any three of the following: uppercase letters, lowercase letters, numbers, special characters. Supported special characters: !@#$%^*()&_+-=.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: AccountPrivileges
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Account permission information. Separate multiple permissions with commas (,). Valid values: Login: Login permission. Inherit: Inherit permission. CreateRole: Create role permission. CreateDB: Create database permission. Note: If the account type is high privilege, you do not need to provide this parameter; all permissions are supported by default. If the account type is normal, you can provide this parameter; the default values are Login and Inherit. If the account type is instance read-only, that is, AccountType is set to InstanceReadOnly, this parameter is not supported.",
		//	  "type": "string"
		//	}
		"account_privileges": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Account permission information. Separate multiple permissions with commas (,). Valid values: Login: Login permission. Inherit: Inherit permission. CreateRole: Create role permission. CreateDB: Create database permission. Note: If the account type is high privilege, you do not need to provide this parameter; all permissions are supported by default. If the account type is normal, you can provide this parameter; the default values are Login and Inherit. If the account type is instance read-only, that is, AccountType is set to InstanceReadOnly, this parameter is not supported.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: AccountType
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Database account type. Valid values: Super: High privilege account. Normal: Normal account. InstanceReadOnly: Instance read-only account.",
		//	  "type": "string"
		//	}
		"account_type": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Database account type. Valid values: Super: High privilege account. Normal: Normal account. InstanceReadOnly: Instance read-only account.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: InstanceId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Instance ID",
		//	  "type": "string"
		//	}
		"instance_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Instance ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NotAllowPrivileges
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Permissions disabled for the account. Currently, only DDL is supported. Note: This field can only be provided for high privilege or normal accounts, that is, when AccountType is set to Super or Normal.",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "type": "string"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"not_allow_privileges": schema.SetAttribute{ /*START ATTRIBUTE*/
			ElementType: types.StringType,
			Description: "Permissions disabled for the account. Currently, only DDL is supported. Note: This field can only be provided for high privilege or normal accounts, that is, when AccountType is set to Super or Normal.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for Byteplus::RDSPostgreSQL::DBAccount",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudControlTypeName("Byteplus::RDSPostgreSQL::DBAccount").WithTerraformTypeName("bytepluscc_rdspostgresql_db_account")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"account_name":         "AccountName",
		"account_password":     "AccountPassword",
		"account_privileges":   "AccountPrivileges",
		"account_type":         "AccountType",
		"instance_id":          "InstanceId",
		"not_allow_privileges": "NotAllowPrivileges",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
