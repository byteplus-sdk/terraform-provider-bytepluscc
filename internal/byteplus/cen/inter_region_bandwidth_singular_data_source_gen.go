// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package cen

import (
	"context"

	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/generic"
	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/registry"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
)

func init() {
	registry.AddDataSourceFactory("bytepluscc_cen_inter_region_bandwidth", interRegionBandwidthDataSource)
}

// interRegionBandwidthDataSource returns the Terraform bytepluscc_cen_inter_region_bandwidth data source.
// This Terraform data source corresponds to the Cloud Control Byteplus::CEN::InterRegionBandwidth resource.
func interRegionBandwidthDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: Bandwidth
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Peak bandwidth of CEN inter-region bandwidth, measured in Mbps. The value cannot exceed the bandwidth that can be allocated by the associated bandwidth package.",
		//	  "type": "integer"
		//	}
		"bandwidth": schema.Int64Attribute{ /*START ATTRIBUTE*/
			Description: "Peak bandwidth of CEN inter-region bandwidth, measured in Mbps. The value cannot exceed the bandwidth that can be allocated by the associated bandwidth package.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CenBandwidthPackageId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Instance ID of the bandwidth package.",
		//	  "type": "string"
		//	}
		"cen_bandwidth_package_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Instance ID of the bandwidth package.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CenId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the Cloud Enterprise Network instance.",
		//	  "type": "string"
		//	}
		"cen_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the Cloud Enterprise Network instance.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CreationTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Time when the CEN inter-region bandwidth was created.",
		//	  "type": "string"
		//	}
		"creation_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Time when the CEN inter-region bandwidth was created.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: InterRegionBandwidthId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the CEN inter-region bandwidth.",
		//	  "type": "string"
		//	}
		"inter_region_bandwidth_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the CEN inter-region bandwidth.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: LocalRegionId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the local region for CEN inter-region bandwidth.",
		//	  "type": "string"
		//	}
		"local_region_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the local region for CEN inter-region bandwidth.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: PeerRegionId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the remote region for CEN inter-region bandwidth.",
		//	  "type": "string"
		//	}
		"peer_region_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the remote region for CEN inter-region bandwidth.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Status
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Status of CEN inter-region bandwidth. Creating: Being created. Deleting: Being deleted. Pending: Configuring. Available: Available.",
		//	  "type": "string"
		//	}
		"status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Status of CEN inter-region bandwidth. Creating: Being created. Deleting: Being deleted. Pending: Configuring. Available: Available.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: UpdateTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Time when the CEN inter-region bandwidth was updated.",
		//	  "type": "string"
		//	}
		"update_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Time when the CEN inter-region bandwidth was updated.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for Byteplus::CEN::InterRegionBandwidth",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudControlTypeName("Byteplus::CEN::InterRegionBandwidth").WithTerraformTypeName("bytepluscc_cen_inter_region_bandwidth")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"bandwidth":                 "Bandwidth",
		"cen_bandwidth_package_id":  "CenBandwidthPackageId",
		"cen_id":                    "CenId",
		"creation_time":             "CreationTime",
		"inter_region_bandwidth_id": "InterRegionBandwidthId",
		"local_region_id":           "LocalRegionId",
		"peer_region_id":            "PeerRegionId",
		"status":                    "Status",
		"update_time":               "UpdateTime",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
