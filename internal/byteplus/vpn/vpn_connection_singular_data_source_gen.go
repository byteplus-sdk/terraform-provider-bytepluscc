// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package vpn

import (
	"context"

	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/generic"
	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/registry"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

func init() {
	registry.AddDataSourceFactory("bytepluscc_vpn_vpn_connection", vPNConnectionDataSource)
}

// vPNConnectionDataSource returns the Terraform bytepluscc_vpn_vpn_connection data source.
// This Terraform data source corresponds to the Cloud Control Byteplus::VPN::VPNConnection resource.
func vPNConnectionDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: AccountId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the account to which the IPsec connection belongs.",
		//	  "type": "string"
		//	}
		"account_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the account to which the IPsec connection belongs.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: AttachStatus
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "IPsec connection status, VpnGateway: Attached: Bound; TransitRouter: Attached: Bound; Attaching: Binding; Detached: Unbound; Detaching: Unbinding.",
		//	  "type": "string"
		//	}
		"attach_status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "IPsec connection status, VpnGateway: Attached: Bound; TransitRouter: Attached: Bound; Attaching: Binding; Detached: Unbound; Detaching: Unbinding.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: AttachType
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Type of gateway associated with the IPsec connection. VpnGateway: VPN gateway; TransitRouter: transit router.",
		//	  "type": "string"
		//	}
		"attach_type": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Type of gateway associated with the IPsec connection. VpnGateway: VPN gateway; TransitRouter: transit router.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: BgpInfo
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "BGP information for the IPsec connection.",
		//	  "properties": {
		//	    "EnableBgp": {
		//	      "description": "Enable dynamic route propagation. false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
		//	      "type": "boolean"
		//	    },
		//	    "LocalAsn": {
		//	      "description": "The ASN of the VPN gateway.",
		//	      "format": "int64",
		//	      "type": "integer"
		//	    },
		//	    "LocalBgpIp": {
		//	      "description": "The local tunnel IP, which is the BGP address configured on the VPN gateway and defaults to the first host address of BgpConfig.TunnelCidr. This address must be an IP address within the IPsec tunnel subnet. If BgpConfig.EnableBgp is set to true, this parameter must be provided.",
		//	      "type": "string"
		//	    },
		//	    "PeerAsn": {
		//	      "description": "ASN of the customer gateway.",
		//	      "format": "int64",
		//	      "type": "integer"
		//	    },
		//	    "PeerBgpIp": {
		//	      "description": "BGP peer IP, that is, the BGP address on the customer gateway side.",
		//	      "type": "string"
		//	    },
		//	    "SessionStatus": {
		//	      "description": "Status of the BGP connection. Up: BGP connection is normal. Down: BGP connection is not working.",
		//	      "enum": [
		//	        "Up",
		//	        "Down"
		//	      ],
		//	      "type": "string"
		//	    },
		//	    "TunnelCidr": {
		//	      "description": "CIDR address range for the local and peer IPs of the BGP session.",
		//	      "type": "string"
		//	    }
		//	  },
		//	  "type": "object"
		//	}
		"bgp_info": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: EnableBgp
				"enable_bgp": schema.BoolAttribute{ /*START ATTRIBUTE*/
					Description: "Enable dynamic route propagation. false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: LocalAsn
				"local_asn": schema.Int64Attribute{ /*START ATTRIBUTE*/
					Description: "The ASN of the VPN gateway.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: LocalBgpIp
				"local_bgp_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "The local tunnel IP, which is the BGP address configured on the VPN gateway and defaults to the first host address of BgpConfig.TunnelCidr. This address must be an IP address within the IPsec tunnel subnet. If BgpConfig.EnableBgp is set to true, this parameter must be provided.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: PeerAsn
				"peer_asn": schema.Int64Attribute{ /*START ATTRIBUTE*/
					Description: "ASN of the customer gateway.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: PeerBgpIp
				"peer_bgp_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "BGP peer IP, that is, the BGP address on the customer gateway side.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: SessionStatus
				"session_status": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Status of the BGP connection. Up: BGP connection is normal. Down: BGP connection is not working.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: TunnelCidr
				"tunnel_cidr": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "CIDR address range for the local and peer IPs of the BGP session.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "BGP information for the IPsec connection.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: BusinessStatus
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Billing status of the IPsec connection. This parameter is returned only when AttachType is 'TransitRouter'. Normal: Normal. FinancialLocked: Financially locked.",
		//	  "type": "string"
		//	}
		"business_status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Billing status of the IPsec connection. This parameter is returned only when AttachType is 'TransitRouter'. Normal: Normal. FinancialLocked: Financially locked.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ConnectStatus
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Status of the tunnel connection. ike_sa_negotiation_failed: Phase 1 negotiation failed. ike_sa_negotiation_completed: Phase 1 negotiation succeeded. ipsec_sa_negotiation_failed: Phase 2 negotiation failed. ipsec_sa_negotiation_completed: Phase 2 negotiation succeeded.",
		//	  "type": "string"
		//	}
		"connect_status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Status of the tunnel connection. ike_sa_negotiation_failed: Phase 1 negotiation failed. ike_sa_negotiation_completed: Phase 1 negotiation succeeded. ipsec_sa_negotiation_failed: Phase 2 negotiation failed. ipsec_sa_negotiation_completed: Phase 2 negotiation succeeded.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CreationTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Time when the IPsec connection was created.",
		//	  "type": "string"
		//	}
		"creation_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Time when the IPsec connection was created.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CustomerGatewayId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the customer gateway associated with the IPsec connection. This parameter is required when creating a single-tunnel IPsec connection. If the customer gateway IP address is 0.0.0.0, NegotiateInstantly can only be set to false, IkeConfig.Version can only be set to ikev1, and IkeConfig.Mode can only be set to aggressive in IkeConfig.",
		//	  "type": "string"
		//	}
		"customer_gateway_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the customer gateway associated with the IPsec connection. This parameter is required when creating a single-tunnel IPsec connection. If the customer gateway IP address is 0.0.0.0, NegotiateInstantly can only be set to false, IkeConfig.Version can only be set to ikev1, and IkeConfig.Mode can only be set to aggressive in IkeConfig.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: DeletedTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Expected resource forced reclamation time. This parameter is returned only when AttachType is 'TransitRouter', and only when the resource is frozen due to overdue payment. Otherwise, the value is empty.",
		//	  "type": "string"
		//	}
		"deleted_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Expected resource forced reclamation time. This parameter is returned only when AttachType is 'TransitRouter', and only when the resource is frozen due to overdue payment. Otherwise, the value is empty.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Description
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Description of the IPsec connection. Must start with a Chinese character, letter, or number, and can only contain Chinese characters, letters, numbers, period (.), space ( ), underscore (_), hyphen (-), equals sign (=), English comma (,), Chinese comma (，), and Chinese period (。). Length limit: 0–255 characters. If not specified, defaults to an empty string.",
		//	  "type": "string"
		//	}
		"description": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Description of the IPsec connection. Must start with a Chinese character, letter, or number, and can only contain Chinese characters, letters, numbers, period (.), space ( ), underscore (_), hyphen (-), equals sign (=), English comma (,), Chinese comma (，), and Chinese period (。). Length limit: 0–255 characters. If not specified, defaults to an empty string.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: DpdAction
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Status of the DPD feature. none: DPD feature disabled. clear (default): DPD feature enabled; IKE SA is cleared after DPD timeout. hold: DPD feature enabled; current state is maintained after DPD timeout; the connection is re-established when new traffic is detected. restart: DPD feature enabled; reconnect immediately after DPD timeout.",
		//	  "type": "string"
		//	}
		"dpd_action": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Status of the DPD feature. none: DPD feature disabled. clear (default): DPD feature enabled; IKE SA is cleared after DPD timeout. hold: DPD feature enabled; current state is maintained after DPD timeout; the connection is re-established when new traffic is detected. restart: DPD feature enabled; reconnect immediately after DPD timeout.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: DualTunnelEnabled
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Enable dual tunnel feature. true: Enabled. false: Not enabled.",
		//	  "type": "boolean"
		//	}
		"dual_tunnel_enabled": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Enable dual tunnel feature. true: Enabled. false: Not enabled.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: EnableTunnelsBgp
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Whether to enable BGP. Valid values: false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
		//	  "type": "boolean"
		//	}
		"enable_tunnels_bgp": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Whether to enable BGP. Valid values: false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: HealthCheckers
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Health check information for the IPsec connection",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "properties": {
		//	      "CheckInterval": {
		//	        "description": "Interval for performing health checks",
		//	        "format": "int64",
		//	        "type": "integer"
		//	      },
		//	      "CheckResult": {
		//	        "description": "Health check result. Up: Normal. Down: Abnormal.",
		//	        "enum": [
		//	          "Up",
		//	          "Down"
		//	        ],
		//	        "type": "string"
		//	      },
		//	      "CheckerId": {
		//	        "description": "ID of the IPsec connection health check",
		//	        "type": "string"
		//	      },
		//	      "DownTime": {
		//	        "description": "Health check unhealthy threshold. If the health check result is 'abnormal' for the specified number of consecutive times, the IPsec connection is considered unhealthy.",
		//	        "format": "int64",
		//	        "type": "integer"
		//	      },
		//	      "LocalIp": {
		//	        "description": "Health check source IP.",
		//	        "type": "string"
		//	      },
		//	      "RemoteIp": {
		//	        "description": "Health check target IP.",
		//	        "type": "string"
		//	      },
		//	      "Timeout": {
		//	        "description": "Health check response timeout. If the IPsec connection does not respond correctly within the specified response time, it is considered 'abnormal' for health check.",
		//	        "format": "int64",
		//	        "type": "integer"
		//	      },
		//	      "UpTime": {
		//	        "description": "Health check threshold. If the specified number of consecutive health checks are all 'healthy', the IPsec connection is considered 'healthy' by health check.",
		//	        "format": "int64",
		//	        "type": "integer"
		//	      }
		//	    },
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"health_checkers": schema.SetNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: CheckInterval
					"check_interval": schema.Int64Attribute{ /*START ATTRIBUTE*/
						Description: "Interval for performing health checks",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: CheckResult
					"check_result": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Health check result. Up: Normal. Down: Abnormal.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: CheckerId
					"checker_id": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "ID of the IPsec connection health check",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: DownTime
					"down_time": schema.Int64Attribute{ /*START ATTRIBUTE*/
						Description: "Health check unhealthy threshold. If the health check result is 'abnormal' for the specified number of consecutive times, the IPsec connection is considered unhealthy.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: LocalIp
					"local_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Health check source IP.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: RemoteIp
					"remote_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Health check target IP.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Timeout
					"timeout": schema.Int64Attribute{ /*START ATTRIBUTE*/
						Description: "Health check response timeout. If the IPsec connection does not respond correctly within the specified response time, it is considered 'abnormal' for health check.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: UpTime
					"up_time": schema.Int64Attribute{ /*START ATTRIBUTE*/
						Description: "Health check threshold. If the specified number of consecutive health checks are all 'healthy', the IPsec connection is considered 'healthy' by health check.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Health check information for the IPsec connection",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: IkeConfig
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "IKE configuration information for the IPsec connection",
		//	  "properties": {
		//	    "AuthAlg": {
		//	      "description": "Authentication algorithm for phase one.",
		//	      "type": "string"
		//	    },
		//	    "DhGroup": {
		//	      "description": "DH (Diffie-Hellman) key exchange algorithm used in phase 1 negotiation. Valid values: group1, group2 (default), group5, group14",
		//	      "type": "string"
		//	    },
		//	    "EncAlg": {
		//	      "description": "Encryption algorithm for phase 1 negotiation.",
		//	      "type": "string"
		//	    },
		//	    "Lifetime": {
		//	      "description": "Lifetime of the SA for phase 1 negotiation. After the lifetime is exceeded, negotiation restarts. Value range: 900~86400, unit: seconds. Default: 86400.",
		//	      "format": "int64",
		//	      "type": "integer"
		//	    },
		//	    "LocalId": {
		//	      "description": "Identifier of the VPN gateway, used for phase 1 negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the current VPN gateway's egress IP address. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ; and digits are allowed.",
		//	      "type": "string"
		//	    },
		//	    "Mode": {
		//	      "description": "Negotiation mode for phase 1. This parameter is required only when IkeConfig.Version is ikev1. Valid values: main (default), aggressive",
		//	      "type": "string"
		//	    },
		//	    "Psk": {
		//	      "description": "Pre-shared key used for identity authentication between the VPN gateway and the user gateway in phase 1. Must not exceed 100 characters and can only contain uppercase and lowercase letters, special symbols ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ;, and numbers.",
		//	      "type": "string"
		//	    },
		//	    "RemoteId": {
		//	      "description": "Identifier of the customer gateway, used for phase one negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the public IP address of the currently selected customer gateway. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ;, and digits are allowed.",
		//	      "type": "string"
		//	    },
		//	    "Version": {
		//	      "default": "ikev2",
		//	      "description": "Version of the IKE key exchange protocol. Valid values: ikev1, ikev2 (default).",
		//	      "type": "string"
		//	    }
		//	  },
		//	  "type": "object"
		//	}
		"ike_config": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: AuthAlg
				"auth_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Authentication algorithm for phase one.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: DhGroup
				"dh_group": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "DH (Diffie-Hellman) key exchange algorithm used in phase 1 negotiation. Valid values: group1, group2 (default), group5, group14",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: EncAlg
				"enc_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Encryption algorithm for phase 1 negotiation.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Lifetime
				"lifetime": schema.Int64Attribute{ /*START ATTRIBUTE*/
					Description: "Lifetime of the SA for phase 1 negotiation. After the lifetime is exceeded, negotiation restarts. Value range: 900~86400, unit: seconds. Default: 86400.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: LocalId
				"local_id": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Identifier of the VPN gateway, used for phase 1 negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the current VPN gateway's egress IP address. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ; and digits are allowed.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Mode
				"mode": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Negotiation mode for phase 1. This parameter is required only when IkeConfig.Version is ikev1. Valid values: main (default), aggressive",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Psk
				"psk": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Pre-shared key used for identity authentication between the VPN gateway and the user gateway in phase 1. Must not exceed 100 characters and can only contain uppercase and lowercase letters, special symbols ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ;, and numbers.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: RemoteId
				"remote_id": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Identifier of the customer gateway, used for phase one negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the public IP address of the currently selected customer gateway. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ;, and digits are allowed.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Version
				"version": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Version of the IKE key exchange protocol. Valid values: ikev1, ikev2 (default).",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "IKE configuration information for the IPsec connection",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: IpAddress
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Public IP address of the transit router. This parameter is returned only when AttachType is 'TransitRouter'.",
		//	  "type": "string"
		//	}
		"ip_address": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Public IP address of the transit router. This parameter is returned only when AttachType is 'TransitRouter'.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: IpsecConfig
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "IPsec configuration information for the IPsec connection.",
		//	  "properties": {
		//	    "AuthAlg": {
		//	      "description": "Authentication algorithm for phase 2 negotiation",
		//	      "type": "string"
		//	    },
		//	    "DhGroup": {
		//	      "description": "DH (Diffie-Hellman) key exchange algorithm used in phase 2",
		//	      "type": "string"
		//	    },
		//	    "EncAlg": {
		//	      "description": "Encryption algorithm for phase 2 negotiation",
		//	      "type": "string"
		//	    },
		//	    "Lifetime": {
		//	      "description": "Lifetime of the SA for phase 2 negotiation. Negotiation will be re-initiated after the lifetime expires.",
		//	      "format": "int64",
		//	      "type": "integer"
		//	    }
		//	  },
		//	  "type": "object"
		//	}
		"ipsec_config": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: AuthAlg
				"auth_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Authentication algorithm for phase 2 negotiation",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: DhGroup
				"dh_group": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "DH (Diffie-Hellman) key exchange algorithm used in phase 2",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: EncAlg
				"enc_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Encryption algorithm for phase 2 negotiation",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: Lifetime
				"lifetime": schema.Int64Attribute{ /*START ATTRIBUTE*/
					Description: "Lifetime of the SA for phase 2 negotiation. Negotiation will be re-initiated after the lifetime expires.",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "IPsec configuration information for the IPsec connection.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: IsBlocked
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Whether the current resource is blocked. true: Blocked; false: Not blocked",
		//	  "type": "boolean"
		//	}
		"is_blocked": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Whether the current resource is blocked. true: Blocked; false: Not blocked",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: LocalSubnet
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Local network segment of the IPsec connection.",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "type": "string"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"local_subnet": schema.SetAttribute{ /*START ATTRIBUTE*/
			ElementType: types.StringType,
			Description: "Local network segment of the IPsec connection.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: LogEnabled
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Enable connection logs. After connection logs are enabled, you can view and download IPsec connection logs and troubleshoot IPsec connection issues using the log information. true: Enabled. false: Not enabled.",
		//	  "type": "boolean"
		//	}
		"log_enabled": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Enable connection logs. After connection logs are enabled, you can view and download IPsec connection logs and troubleshoot IPsec connection issues using the log information. true: Enabled. false: Not enabled.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NatTraversal
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Enable NAT traversal. true: Enabled. When NAT traversal is enabled, IKE negotiation does not verify UDP port numbers and can detect NAT gateway devices in the VPN connection tunnel. false (default): Not enabled.",
		//	  "type": "boolean"
		//	}
		"nat_traversal": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Enable NAT traversal. true: Enabled. When NAT traversal is enabled, IKE negotiation does not verify UDP port numbers and can detect NAT gateway devices in the VPN connection tunnel. false (default): Not enabled.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NegotiateInstantly
		// Cloud Control resource type schema:
		//
		//	{
		//	  "default": false,
		//	  "description": "Whether to initiate negotiation mode immediately. true: The system actively initiates negotiation after the IPsec connection is configured. false (default): The system does not actively initiate negotiation after configuration; negotiation is initiated passively when traffic passes through.",
		//	  "type": "boolean"
		//	}
		"negotiate_instantly": schema.BoolAttribute{ /*START ATTRIBUTE*/
			Description: "Whether to initiate negotiation mode immediately. true: The system actively initiates negotiation after the IPsec connection is configured. false (default): The system does not actively initiate negotiation after configuration; negotiation is initiated passively when traffic passes through.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: OverdueTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Resource freeze time. This parameter is returned only when AttachType is 'TransitRouter', and only when the resource is frozen due to overdue payment. Otherwise, this parameter is empty.",
		//	  "type": "string"
		//	}
		"overdue_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Resource freeze time. This parameter is returned only when AttachType is 'TransitRouter', and only when the resource is frozen due to overdue payment. Otherwise, this parameter is empty.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ProjectName
		// Cloud Control resource type schema:
		//
		//	{
		//	  "default": "default",
		//	  "description": "Project to which the IPsec connection belongs.",
		//	  "type": "string"
		//	}
		"project_name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Project to which the IPsec connection belongs.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: RemoteSubnet
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Peer CIDR block of the IPsec connection",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "type": "string"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"remote_subnet": schema.SetAttribute{ /*START ATTRIBUTE*/
			ElementType: types.StringType,
			Description: "Peer CIDR block of the IPsec connection",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Spec
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "IPsec connection instance specification. Only effective for IPsec associated with TR instances; ineffective for IPsec associated with VPN gateway instances. This parameter takes effect only when AttachType is set to TransitRouter. default (default): Default specification, bandwidth limit is 200 Mbps. large: Large specification, bandwidth limit is 1 Gbps. xlarge: Extra large specification, bandwidth limit is 3 Gbps.",
		//	  "type": "string"
		//	}
		"spec": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "IPsec connection instance specification. Only effective for IPsec associated with TR instances; ineffective for IPsec associated with VPN gateway instances. This parameter takes effect only when AttachType is set to TransitRouter. default (default): Default specification, bandwidth limit is 200 Mbps. large: Large specification, bandwidth limit is 1 Gbps. xlarge: Extra large specification, bandwidth limit is 3 Gbps.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Status
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Status of the IPsec connection. Creating: Creating. Deleting: Deleting. Pending: Configuring. Available: Available.",
		//	  "type": "string"
		//	}
		"status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Status of the IPsec connection. Creating: Creating. Deleting: Deleting. Pending: Configuring. Available: Available.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Tags
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "All tag information added to the IPsec connection.",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "properties": {
		//	      "Key": {
		//	        "description": "Tag key.",
		//	        "type": "string"
		//	      },
		//	      "Value": {
		//	        "description": "Tag value",
		//	        "type": "string"
		//	      }
		//	    },
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"tags": schema.SetNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Key
					"key": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag key.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Value
					"value": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag value",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "All tag information added to the IPsec connection.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: TransitRouterId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "The ID of the transit router. This parameter is returned only when AttachType is 'TransitRouter'.",
		//	  "type": "string"
		//	}
		"transit_router_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The ID of the transit router. This parameter is returned only when AttachType is 'TransitRouter'.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: TunnelOptions
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Tunnel configuration information for the IPsec connection.",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "properties": {
		//	      "ConnectStatus": {
		//	        "description": "IPsec tunnel status. ike_sa_negotiation_failed: Phase one negotiation failed; nike_sa_negotiation_completed: Phase one negotiation succeeded; nipsec_sa_negotiation_failed: Phase two negotiation failed; nipsec_sa_negotiation_completed: Phase two negotiation succeeded.",
		//	        "type": "string"
		//	      },
		//	      "CustomerGatewayId": {
		//	        "description": "ID of the customer gateway associated with the IPsec connection.",
		//	        "type": "string"
		//	      },
		//	      "DpdAction": {
		//	        "description": "Status of the DPD feature.",
		//	        "type": "string"
		//	      },
		//	      "IkeConfig": {
		//	        "description": "IKE configuration information for the IPsec connection.",
		//	        "properties": {
		//	          "AuthAlg": {
		//	            "description": "Authentication algorithm for phase one.",
		//	            "type": "string"
		//	          },
		//	          "DhGroup": {
		//	            "description": "DH (Diffie-Hellman) key exchange algorithm used in phase 1 negotiation. Valid values: group1, group2 (default), group5, group14",
		//	            "type": "string"
		//	          },
		//	          "EncAlg": {
		//	            "description": "Encryption algorithm for phase 1 negotiation.",
		//	            "type": "string"
		//	          },
		//	          "Lifetime": {
		//	            "description": "Lifetime of the SA for phase 1 negotiation. After the lifetime is exceeded, negotiation restarts. Value range: 900~86400, unit: seconds. Default: 86400.",
		//	            "format": "int64",
		//	            "type": "integer"
		//	          },
		//	          "LocalId": {
		//	            "description": "Identifier of the VPN gateway, used for phase 1 negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the current VPN gateway's egress IP address. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ; and digits are allowed.",
		//	            "type": "string"
		//	          },
		//	          "Mode": {
		//	            "description": "Negotiation mode for phase 1. This parameter is required only when IkeConfig.Version is ikev1. Valid values: main (default), aggressive",
		//	            "type": "string"
		//	          },
		//	          "Psk": {
		//	            "description": "Pre-shared key used for identity authentication between the VPN gateway and the user gateway in phase 1. Must not exceed 100 characters and can only contain uppercase and lowercase letters, special symbols ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ;, and numbers.",
		//	            "type": "string"
		//	          },
		//	          "RemoteId": {
		//	            "description": "Identifier of the customer gateway, used for phase one negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the public IP address of the currently selected customer gateway. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( ) - _ + = [ ] { } \\ , . / : ;, and digits are allowed.",
		//	            "type": "string"
		//	          },
		//	          "Version": {
		//	            "default": "ikev2",
		//	            "description": "Version of the IKE key exchange protocol. Valid values: ikev1, ikev2 (default).",
		//	            "type": "string"
		//	          }
		//	        },
		//	        "type": "object"
		//	      },
		//	      "IpsecConfig": {
		//	        "description": "Information about the IPsec configuration in the IPsec connection.",
		//	        "properties": {
		//	          "AuthAlg": {
		//	            "description": "Authentication algorithm for phase 2 negotiation",
		//	            "type": "string"
		//	          },
		//	          "DhGroup": {
		//	            "description": "DH (Diffie-Hellman) key exchange algorithm used in phase 2",
		//	            "type": "string"
		//	          },
		//	          "EncAlg": {
		//	            "description": "Encryption algorithm for phase 2 negotiation",
		//	            "type": "string"
		//	          },
		//	          "Lifetime": {
		//	            "description": "Lifetime of the SA for phase 2 negotiation. Negotiation will be re-initiated after the lifetime expires.",
		//	            "format": "int64",
		//	            "type": "integer"
		//	          }
		//	        },
		//	        "type": "object"
		//	      },
		//	      "NatTraversal": {
		//	        "description": "Whether to enable NAT traversal",
		//	        "type": "boolean"
		//	      },
		//	      "Role": {
		//	        "description": "The role of the tunnel.",
		//	        "type": "string"
		//	      },
		//	      "TunnelBgpInfo": {
		//	        "description": "BGP session information.",
		//	        "properties": {
		//	          "EnableBgp": {
		//	            "description": "Enable dynamic route propagation. false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
		//	            "type": "boolean"
		//	          },
		//	          "LocalAsn": {
		//	            "description": "The ASN of the VPN gateway.",
		//	            "format": "int64",
		//	            "type": "integer"
		//	          },
		//	          "LocalBgpIp": {
		//	            "description": "BGP address. Defaults to the first host address of TunnelOptions.N.BGPConfig.TunnelCidr. This address must be an IP address within the IPsec tunnel CIDR block. If EnableTunnelsBgp is set to true, this parameter must be specified.",
		//	            "type": "string"
		//	          },
		//	          "PeerAsn": {
		//	            "description": "ASN of the customer gateway.",
		//	            "format": "int64",
		//	            "type": "integer"
		//	          },
		//	          "PeerBgpIp": {
		//	            "description": "BGP peer IP, that is, the BGP address on the customer gateway side.",
		//	            "type": "string"
		//	          },
		//	          "SessionStatus": {
		//	            "description": "Status of the BGP connection. Up: BGP connection is normal. Down: BGP connection is not working.",
		//	            "type": "string"
		//	          },
		//	          "TunnelCidr": {
		//	            "description": "The CIDR address range for the local and peer IPs of the BGP session. This range must be within the 169.254.0.0/16 subnet with a subnet mask length of 30. If EnableTunnelsBgp is set to true, this parameter must be provided.",
		//	            "type": "string"
		//	          }
		//	        },
		//	        "type": "object"
		//	      },
		//	      "TunnelId": {
		//	        "description": "Tunnel ID of the IPsec connection.",
		//	        "type": "string"
		//	      }
		//	    },
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": false
		//	}
		"tunnel_options": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: ConnectStatus
					"connect_status": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "IPsec tunnel status. ike_sa_negotiation_failed: Phase one negotiation failed; nike_sa_negotiation_completed: Phase one negotiation succeeded; nipsec_sa_negotiation_failed: Phase two negotiation failed; nipsec_sa_negotiation_completed: Phase two negotiation succeeded.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: CustomerGatewayId
					"customer_gateway_id": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "ID of the customer gateway associated with the IPsec connection.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: DpdAction
					"dpd_action": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Status of the DPD feature.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: IkeConfig
					"ike_config": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
						Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
							// Property: AuthAlg
							"auth_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Authentication algorithm for phase one.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: DhGroup
							"dh_group": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "DH (Diffie-Hellman) key exchange algorithm used in phase 1 negotiation. Valid values: group1, group2 (default), group5, group14",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: EncAlg
							"enc_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Encryption algorithm for phase 1 negotiation.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Lifetime
							"lifetime": schema.Int64Attribute{ /*START ATTRIBUTE*/
								Description: "Lifetime of the SA for phase 1 negotiation. After the lifetime is exceeded, negotiation restarts. Value range: 900~86400, unit: seconds. Default: 86400.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: LocalId
							"local_id": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Identifier of the VPN gateway, used for phase 1 negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the current VPN gateway's egress IP address. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ; and digits are allowed.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Mode
							"mode": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Negotiation mode for phase 1. This parameter is required only when IkeConfig.Version is ikev1. Valid values: main (default), aggressive",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Psk
							"psk": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Pre-shared key used for identity authentication between the VPN gateway and the user gateway in phase 1. Must not exceed 100 characters and can only contain uppercase and lowercase letters, special symbols ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ;, and numbers.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: RemoteId
							"remote_id": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Identifier of the customer gateway, used for phase one negotiation. Supports IP format and FQDN (Fully Qualified Domain Name) format. If not specified, defaults to the public IP address of the currently selected customer gateway. If you manually set it to FQDN format, it is recommended to set the negotiation mode to 'aggressive'. Maximum length is 100 characters. Only uppercase and lowercase letters, special characters ~ ` | ! @ # $ % ^ ( )   - _ + = [ ] { } \\ , . / : ;, and digits are allowed.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Version
							"version": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Version of the IKE key exchange protocol. Valid values: ikev1, ikev2 (default).",
								Computed:    true,
							}, /*END ATTRIBUTE*/
						}, /*END SCHEMA*/
						Description: "IKE configuration information for the IPsec connection.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: IpsecConfig
					"ipsec_config": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
						Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
							// Property: AuthAlg
							"auth_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Authentication algorithm for phase 2 negotiation",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: DhGroup
							"dh_group": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "DH (Diffie-Hellman) key exchange algorithm used in phase 2",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: EncAlg
							"enc_alg": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Encryption algorithm for phase 2 negotiation",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: Lifetime
							"lifetime": schema.Int64Attribute{ /*START ATTRIBUTE*/
								Description: "Lifetime of the SA for phase 2 negotiation. Negotiation will be re-initiated after the lifetime expires.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
						}, /*END SCHEMA*/
						Description: "Information about the IPsec configuration in the IPsec connection.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: NatTraversal
					"nat_traversal": schema.BoolAttribute{ /*START ATTRIBUTE*/
						Description: "Whether to enable NAT traversal",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Role
					"role": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "The role of the tunnel.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: TunnelBgpInfo
					"tunnel_bgp_info": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
						Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
							// Property: EnableBgp
							"enable_bgp": schema.BoolAttribute{ /*START ATTRIBUTE*/
								Description: "Enable dynamic route propagation. false (default): No, use static routing mode. true: Yes, use BGP routing mode.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: LocalAsn
							"local_asn": schema.Int64Attribute{ /*START ATTRIBUTE*/
								Description: "The ASN of the VPN gateway.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: LocalBgpIp
							"local_bgp_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "BGP address. Defaults to the first host address of TunnelOptions.N.BGPConfig.TunnelCidr. This address must be an IP address within the IPsec tunnel CIDR block. If EnableTunnelsBgp is set to true, this parameter must be specified.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: PeerAsn
							"peer_asn": schema.Int64Attribute{ /*START ATTRIBUTE*/
								Description: "ASN of the customer gateway.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: PeerBgpIp
							"peer_bgp_ip": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "BGP peer IP, that is, the BGP address on the customer gateway side.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: SessionStatus
							"session_status": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "Status of the BGP connection. Up: BGP connection is normal. Down: BGP connection is not working.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
							// Property: TunnelCidr
							"tunnel_cidr": schema.StringAttribute{ /*START ATTRIBUTE*/
								Description: "The CIDR address range for the local and peer IPs of the BGP session. This range must be within the 169.254.0.0/16 subnet with a subnet mask length of 30. If EnableTunnelsBgp is set to true, this parameter must be provided.",
								Computed:    true,
							}, /*END ATTRIBUTE*/
						}, /*END SCHEMA*/
						Description: "BGP session information.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: TunnelId
					"tunnel_id": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tunnel ID of the IPsec connection.",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Tunnel configuration information for the IPsec connection.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: UpdateTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "The time when the IPsec connection was updated.",
		//	  "type": "string"
		//	}
		"update_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The time when the IPsec connection was updated.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: VpnConnectionId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "ID of the IPsec connection",
		//	  "type": "string"
		//	}
		"vpn_connection_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "ID of the IPsec connection",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: VpnConnectionName
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Name of the IPsec connection. Must start with a Chinese character, English letter, or number, and can only contain Chinese characters, English letters, numbers, dot (.), underscore (_), and hyphen (-). Length must be between 1 and 128 characters. If not specified, defaults to the IPsec connection ID.",
		//	  "type": "string"
		//	}
		"vpn_connection_name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Name of the IPsec connection. Must start with a Chinese character, English letter, or number, and can only contain Chinese characters, English letters, numbers, dot (.), underscore (_), and hyphen (-). Length must be between 1 and 128 characters. If not specified, defaults to the IPsec connection ID.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: VpnGatewayId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "The ID of the VPN gateway associated with the IPsec connection. If AttachType is not specified or set to VpnGateway, this parameter must be provided. If AttachType is set to TransitRouter, this parameter is not required.",
		//	  "type": "string"
		//	}
		"vpn_gateway_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "The ID of the VPN gateway associated with the IPsec connection. If AttachType is not specified or set to VpnGateway, this parameter must be provided. If AttachType is set to TransitRouter, this parameter is not required.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: ZoneId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Availability zone of the transit router attachment point. This parameter is returned only when AttachType is 'TransitRouter'.",
		//	  "type": "string"
		//	}
		"zone_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Availability zone of the transit router attachment point. This parameter is returned only when AttachType is 'TransitRouter'.",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for Byteplus::VPN::VPNConnection",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudControlTypeName("Byteplus::VPN::VPNConnection").WithTerraformTypeName("bytepluscc_vpn_vpn_connection")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"account_id":          "AccountId",
		"attach_status":       "AttachStatus",
		"attach_type":         "AttachType",
		"auth_alg":            "AuthAlg",
		"bgp_info":            "BgpInfo",
		"business_status":     "BusinessStatus",
		"check_interval":      "CheckInterval",
		"check_result":        "CheckResult",
		"checker_id":          "CheckerId",
		"connect_status":      "ConnectStatus",
		"creation_time":       "CreationTime",
		"customer_gateway_id": "CustomerGatewayId",
		"deleted_time":        "DeletedTime",
		"description":         "Description",
		"dh_group":            "DhGroup",
		"down_time":           "DownTime",
		"dpd_action":          "DpdAction",
		"dual_tunnel_enabled": "DualTunnelEnabled",
		"enable_bgp":          "EnableBgp",
		"enable_tunnels_bgp":  "EnableTunnelsBgp",
		"enc_alg":             "EncAlg",
		"health_checkers":     "HealthCheckers",
		"ike_config":          "IkeConfig",
		"ip_address":          "IpAddress",
		"ipsec_config":        "IpsecConfig",
		"is_blocked":          "IsBlocked",
		"key":                 "Key",
		"lifetime":            "Lifetime",
		"local_asn":           "LocalAsn",
		"local_bgp_ip":        "LocalBgpIp",
		"local_id":            "LocalId",
		"local_ip":            "LocalIp",
		"local_subnet":        "LocalSubnet",
		"log_enabled":         "LogEnabled",
		"mode":                "Mode",
		"nat_traversal":       "NatTraversal",
		"negotiate_instantly": "NegotiateInstantly",
		"overdue_time":        "OverdueTime",
		"peer_asn":            "PeerAsn",
		"peer_bgp_ip":         "PeerBgpIp",
		"project_name":        "ProjectName",
		"psk":                 "Psk",
		"remote_id":           "RemoteId",
		"remote_ip":           "RemoteIp",
		"remote_subnet":       "RemoteSubnet",
		"role":                "Role",
		"session_status":      "SessionStatus",
		"spec":                "Spec",
		"status":              "Status",
		"tags":                "Tags",
		"timeout":             "Timeout",
		"transit_router_id":   "TransitRouterId",
		"tunnel_bgp_info":     "TunnelBgpInfo",
		"tunnel_cidr":         "TunnelCidr",
		"tunnel_id":           "TunnelId",
		"tunnel_options":      "TunnelOptions",
		"up_time":             "UpTime",
		"update_time":         "UpdateTime",
		"value":               "Value",
		"version":             "Version",
		"vpn_connection_id":   "VpnConnectionId",
		"vpn_connection_name": "VpnConnectionName",
		"vpn_gateway_id":      "VpnGatewayId",
		"zone_id":             "ZoneId",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
