// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package cloudmonitor

import (
	"context"

	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/generic"
	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/registry"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
	"github.com/hashicorp/terraform-plugin-framework/types"
)

func init() {
	registry.AddDataSourceFactory("bytepluscc_cloudmonitor_event_rule", eventRuleDataSource)
}

// eventRuleDataSource returns the Terraform bytepluscc_cloudmonitor_event_rule data source.
// This Terraform data source corresponds to the Cloud Control Byteplus::CloudMonitor::EventRule resource.
func eventRuleDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: AccountId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Main account ID",
		//	  "type": "string"
		//	}
		"account_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Main account ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CreatedAt
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event rule creation time",
		//	  "type": "integer"
		//	}
		"created_at": schema.Int64Attribute{ /*START ATTRIBUTE*/
			Description: "Event rule creation time",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Description
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Rule description\nMust not start with a digit, hyphen (-), or Chinese punctuation\nCan only contain Chinese characters, letters, digits, underscore (_), hyphen (-), or Chinese punctuation\nLength limit: 0–255 characters",
		//	  "type": "string"
		//	}
		"description": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Rule description\n  Must not start with a digit, hyphen (-), or Chinese punctuation\n  Can only contain Chinese characters, letters, digits, underscore (_), hyphen (-), or Chinese punctuation\n  Length limit: 0–255 characters",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: EnableState
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Rule status\n\nenable: Rule enabled\ndisable: Rule disabled",
		//	  "type": "string"
		//	}
		"enable_state": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Rule status\n  \n  enable: Rule enabled\n  disable: Rule disabled",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: EventBusName
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event bus name. Currently, only 'default' can be entered",
		//	  "enum": [
		//	    "default"
		//	  ],
		//	  "type": "string"
		//	}
		"event_bus_name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Event bus name. Currently, only 'default' can be entered",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: EventSource
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event source",
		//	  "type": "string"
		//	}
		"event_source": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Event source",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: EventType
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event type",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "type": "string"
		//	  },
		//	  "minItems": 1,
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"event_type": schema.SetAttribute{ /*START ATTRIBUTE*/
			ElementType: types.StringType,
			Description: "Event type",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: FilterPattern
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Filter mode, also known as event matching rule. Read-only",
		//	  "type": "string"
		//	}
		"filter_pattern": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Filter mode, also known as event matching rule. Read-only",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: FilterPatternInput
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Filter mode, also known as event matching rule. Used for creation and modification. Format: {\"Data\":{\"autoscaling\":[\"1\",\"3\"],\"ecs\":[\"2\"]}}",
		//	  "type": "string"
		//	}
		"filter_pattern_input": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Filter mode, also known as event matching rule. Used for creation and modification. Format: {\"Data\":{\"autoscaling\":[\"1\",\"3\"],\"ecs\":[\"2\"]}}",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Level
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule severity\n\nnotice: Notification\nwarning: Warning\ncritical: Critical",
		//	  "type": "string"
		//	}
		"level": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule severity\n  \n  notice: Notification\n  warning: Warning\n  critical: Critical",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NotificationId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Policy ID",
		//	  "type": "string"
		//	}
		"notification_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Policy ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: RuleId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event rule ID",
		//	  "type": "string"
		//	}
		"rule_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Event rule ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: RuleName
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Rule name\nLength limit: 1–128 characters\nMust not start with a digit or hyphen (-)",
		//	  "maxLength": 128,
		//	  "minLength": 1,
		//	  "type": "string"
		//	}
		"rule_name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Rule name\n  Length limit: 1–128 characters\n  Must not start with a digit or hyphen (-)",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: UpdatedAt
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Event rule update time",
		//	  "type": "integer"
		//	}
		"updated_at": schema.Int64Attribute{ /*START ATTRIBUTE*/
			Description: "Event rule update time",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for Byteplus::CloudMonitor::EventRule",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudControlTypeName("Byteplus::CloudMonitor::EventRule").WithTerraformTypeName("bytepluscc_cloudmonitor_event_rule")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"account_id":           "AccountId",
		"created_at":           "CreatedAt",
		"description":          "Description",
		"enable_state":         "EnableState",
		"event_bus_name":       "EventBusName",
		"event_source":         "EventSource",
		"event_type":           "EventType",
		"filter_pattern":       "FilterPattern",
		"filter_pattern_input": "FilterPatternInput",
		"level":                "Level",
		"notification_id":      "NotificationId",
		"rule_id":              "RuleId",
		"rule_name":            "RuleName",
		"updated_at":           "UpdatedAt",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
