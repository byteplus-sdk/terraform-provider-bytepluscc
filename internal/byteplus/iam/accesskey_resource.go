...
// Hand-written customization for `bytepluscc_iam_accesskey`.
//
// The Byteplus Cloud Control API only returns `SecretAccessKey` from
...
const (
	accesskeyTerraformTypeName    = "bytepluscc_iam_accesskey"
	accesskeyCloudControlTypeName = "Byteplus::IAM::Accesskey"
...
// from prior state. The Byteplus IAM API's GetResource never returns
...
// event's ResourceModel so it can extract SecretAccessKey, which the Byteplus
