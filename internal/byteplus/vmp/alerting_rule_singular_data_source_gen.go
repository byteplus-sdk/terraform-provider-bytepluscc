// Copyright (c) HashiCorp, Inc.
// SPDX-License-Identifier: MPL-2.0

// Code generated by generators/singular-data-source/main.go; DO NOT EDIT.

package vmp

import (
	"context"

	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/generic"
	"github.com/byteplus-sdk/terraform-provider-bytepluscc/internal/registry"
	"github.com/hashicorp/terraform-plugin-framework/datasource"
	"github.com/hashicorp/terraform-plugin-framework/datasource/schema"
)

func init() {
	registry.AddDataSourceFactory("bytepluscc_vmp_alerting_rule", alertingRuleDataSource)
}

// alertingRuleDataSource returns the Terraform bytepluscc_vmp_alerting_rule data source.
// This Terraform data source corresponds to the Cloud Control Byteplus::VMP::AlertingRule resource.
func alertingRuleDataSource(ctx context.Context) (datasource.DataSource, error) {
	attributes := map[string]schema.Attribute{ /*START SCHEMA*/
		// Property: AlertingRuleId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule ID",
		//	  "type": "string"
		//	}
		"alerting_rule_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Annotations
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Annotation attached to the alert",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "description": "Tag/annotation key-value pair configuration",
		//	    "properties": {
		//	      "Name": {
		//	        "description": "Tag or annotation key",
		//	        "type": "string"
		//	      },
		//	      "Value": {
		//	        "description": "Tag or annotation value. If set to empty, the key will be deleted",
		//	        "type": "string"
		//	      }
		//	    },
		//	    "required": [
		//	      "Name"
		//	    ],
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"annotations": schema.SetNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Name
					"name": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation key",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Value
					"value": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation value. If set to empty, the key will be deleted",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Annotation attached to the alert",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: CreateTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule creation time",
		//	  "type": "string"
		//	}
		"create_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule creation time",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Description
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule description",
		//	  "type": "string"
		//	}
		"description": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule description",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Labels
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Tags attached to the alert",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "description": "Tag/annotation key-value pair configuration",
		//	    "properties": {
		//	      "Name": {
		//	        "description": "Tag or annotation key",
		//	        "type": "string"
		//	      },
		//	      "Value": {
		//	        "description": "Tag or annotation value. If set to empty, the key will be deleted",
		//	        "type": "string"
		//	      }
		//	    },
		//	    "required": [
		//	      "Name"
		//	    ],
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"labels": schema.SetNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Name
					"name": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation key",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Value
					"value": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation value. If set to empty, the key will be deleted",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Tags attached to the alert",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Levels
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert severity",
		//	  "insertionOrder": true,
		//	  "items": {
		//	    "description": "Alert severity configuration",
		//	    "properties": {
		//	      "Comparator": {
		//	        "description": "Alert rule comparison condition. Options: \u003e, \u003e=, \u003c, \u003c=, ==, !=",
		//	        "type": "string"
		//	      },
		//	      "For": {
		//	        "description": "Alert duration. Possible values: 0s, 1m, 2m, 5m, 10m",
		//	        "type": "string"
		//	      },
		//	      "Level": {
		//	        "description": "Alert rule severity. Currently supports P0, P1, or P2",
		//	        "type": "string"
		//	      },
		//	      "Threshold": {
		//	        "description": "Alert rule threshold",
		//	        "type": "number"
		//	      }
		//	    },
		//	    "required": [
		//	      "Level",
		//	      "For",
		//	      "Comparator"
		//	    ],
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"levels": schema.ListNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Comparator
					"comparator": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Alert rule comparison condition. Options: >, >=, <, <=, ==, !=",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: For
					"for": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Alert duration. Possible values: 0s, 1m, 2m, 5m, 10m",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Level
					"level": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Alert rule severity. Currently supports P0, P1, or P2",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Threshold
					"threshold": schema.Float64Attribute{ /*START ATTRIBUTE*/
						Description: "Alert rule threshold",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Alert severity",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Name
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule name",
		//	  "type": "string"
		//	}
		"name": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule name",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NotifyGroupPolicyId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert aggregation policy ID",
		//	  "type": "string"
		//	}
		"notify_group_policy_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert aggregation policy ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: NotifyPolicyId
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert notification policy ID",
		//	  "type": "string"
		//	}
		"notify_policy_id": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert notification policy ID",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Query
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert configuration",
		//	  "properties": {
		//	    "PromQL": {
		//	      "description": "Query statement",
		//	      "type": "string"
		//	    },
		//	    "WorkspaceId": {
		//	      "description": "Workspace ID",
		//	      "type": "string"
		//	    }
		//	  },
		//	  "required": [
		//	    "WorkspaceId"
		//	  ],
		//	  "type": "object"
		//	}
		"query": schema.SingleNestedAttribute{ /*START ATTRIBUTE*/
			Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
				// Property: PromQL
				"prom_ql": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Query statement",
					Computed:    true,
				}, /*END ATTRIBUTE*/
				// Property: WorkspaceId
				"workspace_id": schema.StringAttribute{ /*START ATTRIBUTE*/
					Description: "Workspace ID",
					Computed:    true,
				}, /*END ATTRIBUTE*/
			}, /*END SCHEMA*/
			Description: "Alert configuration",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Status
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule status: Running (monitoring), Disabled (disabled)",
		//	  "type": "string"
		//	}
		"status": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule status: Running (monitoring), Disabled (disabled)",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Tags
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Tag",
		//	  "insertionOrder": false,
		//	  "items": {
		//	    "description": "Tag/annotation key-value pair configuration",
		//	    "properties": {
		//	      "Key": {
		//	        "description": "Tag or annotation key",
		//	        "type": "string"
		//	      },
		//	      "Value": {
		//	        "description": "Tag or annotation value. If set to empty, the key will be deleted",
		//	        "type": "string"
		//	      }
		//	    },
		//	    "type": "object"
		//	  },
		//	  "type": "array",
		//	  "uniqueItems": true
		//	}
		"tags": schema.SetNestedAttribute{ /*START ATTRIBUTE*/
			NestedObject: schema.NestedAttributeObject{ /*START NESTED OBJECT*/
				Attributes: map[string]schema.Attribute{ /*START SCHEMA*/
					// Property: Key
					"key": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation key",
						Computed:    true,
					}, /*END ATTRIBUTE*/
					// Property: Value
					"value": schema.StringAttribute{ /*START ATTRIBUTE*/
						Description: "Tag or annotation value. If set to empty, the key will be deleted",
						Computed:    true,
					}, /*END ATTRIBUTE*/
				}, /*END SCHEMA*/
			}, /*END NESTED OBJECT*/
			Description: "Tag",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: Type
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule type. Allowed values: vmp/PromQL",
		//	  "type": "string"
		//	}
		"type": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule type. Allowed values: vmp/PromQL",
			Computed:    true,
		}, /*END ATTRIBUTE*/
		// Property: UpdateTime
		// Cloud Control resource type schema:
		//
		//	{
		//	  "description": "Alert rule update time",
		//	  "type": "string"
		//	}
		"update_time": schema.StringAttribute{ /*START ATTRIBUTE*/
			Description: "Alert rule update time",
			Computed:    true,
		}, /*END ATTRIBUTE*/
	} /*END SCHEMA*/

	attributes["id"] = schema.StringAttribute{
		Description: "Uniquely identifies the resource.",
		Required:    true,
	}

	schema := schema.Schema{
		Description: "Data Source schema for Byteplus::VMP::AlertingRule",
		Attributes:  attributes,
	}

	var opts generic.DataSourceOptions

	opts = opts.WithCloudControlTypeName("Byteplus::VMP::AlertingRule").WithTerraformTypeName("bytepluscc_vmp_alerting_rule")
	opts = opts.WithTerraformSchema(schema)
	opts = opts.WithAttributeNameMap(map[string]string{
		"alerting_rule_id":       "AlertingRuleId",
		"annotations":            "Annotations",
		"comparator":             "Comparator",
		"create_time":            "CreateTime",
		"description":            "Description",
		"for":                    "For",
		"key":                    "Key",
		"labels":                 "Labels",
		"level":                  "Level",
		"levels":                 "Levels",
		"name":                   "Name",
		"notify_group_policy_id": "NotifyGroupPolicyId",
		"notify_policy_id":       "NotifyPolicyId",
		"prom_ql":                "PromQL",
		"query":                  "Query",
		"status":                 "Status",
		"tags":                   "Tags",
		"threshold":              "Threshold",
		"type":                   "Type",
		"update_time":            "UpdateTime",
		"value":                  "Value",
		"workspace_id":           "WorkspaceId",
	})

	v, err := generic.NewSingularDataSource(ctx, opts...)

	if err != nil {
		return nil, err
	}

	return v, nil
}
